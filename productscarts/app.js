...
function getCartByCustomerId(customerId) {
  let cart = carts.filter(
    cart => cart.customer_id === customerId
  )
  return cart
}

function addCartByCustomerId(customerId, cartData) {
  let prevCarts = carts.filter(
    cart => cart.customer_id !== customerId
  )
  let newCarts = prevCarts.push(cartData)
  return newCarts
}

function emptyCartByCustomerId(customerId) {
  let newCarts = carts.filter(
    cart => cart.customer_id !== customerId
  )
  return newCarts
}
...
app.get('/api/cart/:customerId', function (req, res) {
  let customerId=req.params.customerId
  let cart=getCartByCustomerId(customerId)
  res.send(cart)
})

app.get('/api/cart/add/:customerId/:productId', function (req, res) {
  console.log(req.params)
  let customerId=req.params.customerId
  let productId=req.params.productId
  let newCart=JSON.parse(JSON.stringify(getCart()));// deep copy
...
app.get('/api/cart/update/:customerId/:lineItemId/:quantity', function (req, res) {
  let lineItemId=req.params.lineItemId
  let quantity=req.params.quantity
